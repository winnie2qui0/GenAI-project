...
class Listing(Base):
    ...
    price_normalized_usd: Mapped[Decimal | None] = mapped_column(Numeric(10, 2), nullable=True)
    ...
    seller_rating: Mapped[Decimal | None] = mapped_column(Numeric(2, 1), nullable=True)
    raw_data: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    crawl_time: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    ...
    embedding: Mapped["Embedding | None"] = relationship(
        "Embedding", back_populates="listing", uselist=False, cascade="all, delete-orphan"
    )
    scoring_results: Mapped[list["ScoringResult"]] = relationship(
        "ScoringResult", back_populates="listing", cascade="all, delete-orphan"
    )
